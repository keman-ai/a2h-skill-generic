...
- 🔴 **降级只对只读请求（GET/HEAD）生效**。`listing create`、`message send` 这类**写操作
  只发一发**，第一档失败就直接报错、绝不换出口重发——连接层失败证明不了服务端没收到，
  自动重发同一个 body 会真的多建一条帖/一条私信（2026-08 出过这个事故）。
  写操作报 `network` 错误时，正文会说**这一笔可能已经生效**：先用 `listing mine` /
  `message mine` 查一下在不在，**确认没生效再重发**，别照着"网络错误"直接重来一遍；
- 只有代理能出网的环境里要发帖/发私信 → 显式 `A2HMARKET_PROXY_MODE=auto`（链首就是系统出口，
  仍然只发一发）；
